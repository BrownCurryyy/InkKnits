...
async def register(payload: AuthRegister, db: Session = Depends(get_db), current_user: User | None = Depends(get_optional_current_user)) -> TokenOut:
    ...
    if existing_user is not None:
        if existing_user.organization_id != payload.organization_id or existing_user.status != "REMOVED":
            raise HTTPException(status_code=409, detail="A user with this email already exists")
        existing_user.display_name = payload.display_name
        existing_user.password_hash = hash_password(payload.password)
        existing_user.status = "ACTIVE"
        existing_user.deleted_at = None
        existing_roles = db.query(UserRole).join(Role, Role.id == UserRole.role_id).filter(
            UserRole.user_id == existing_user.id,
            Role.organization_id == payload.organization_id,
        ).all()
        for user_role in existing_roles:
            db.delete(user_role)
        db.flush()
        ensure_default_user_role(db, existing_user, role_name=role_name)
        db.commit()
