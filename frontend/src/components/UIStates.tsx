...
export function CozySkeleton({ rows = 3 }: { rows?: number }) {
  return (
    <div className="animate-pulse overflow-hidden rounded-[18px] border border-[#D9D6CF] bg-[#F5F3EE]/90 p-5 shadow-[0_12px_24px_rgba(13,13,13,0.04)] dark:border-[#292929] dark:bg-[#151515]/90" aria-label="Loading">
      <div className="h-5 w-36 rounded-full bg-accent/15" />
      {Array.from({ length: rows }, (_, index) => (
        <div key={index} className="mt-4 rounded-[14px] border border-[#D9D6CF] bg-[#F5F3EE] p-4 dark:border-[#292929] dark:bg-[#1A1A1A]/80">
...
export function CozyEmptyState({ title, message, icon = '•' }: EmptyStateProps) {
  return (
    <div className="rounded-[18px] border border-dashed border-[#D9D6CF] bg-[#F5F3EE]/80 px-6 py-10 text-center transition-colors duration-200 dark:border-[#292929] dark:bg-[#1A1A1A]/60">
