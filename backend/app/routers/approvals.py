...
async def list_approvals(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[ApprovalTaskOut]:
    ...
    for task in repository.list_all():
        try:
            ...
            if scoped_task.assigned_to == current_user.id or scoped_task.assigned_by == current_user.id or roles.intersection({"ADMIN"}):
...
async def approve_task(
    ...
    require_roles(get_user_roles(db, current_user), ("ADMIN", "REVIEWER"))
    """Approve the asset linked to this approval task."""
    task_uuid = parse_uuid(task_id, "task_id")
    task, asset = get_scoped_task(task_uuid, db, current_user)
    if task.assigned_to != current_user.id and not {role.upper() for role in get_user_roles(db, current_user)}.intersection({"ADMIN"}):
